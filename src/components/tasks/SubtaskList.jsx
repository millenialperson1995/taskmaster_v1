...
export const SubtaskList = ({ task }) => {
  ...
  const handleAddSubtask = (e) => { e.preventDefault(); if (!subtaskText.trim()) return; handleUpdate({ ...task, subtasks: [...task.subtasks, { id: Date.now(), text: subtaskText, completed: false }] }); setSubtaskText(''); };
  const handleToggleSubtask = (subtaskId) => { handleUpdate({ ...task, subtasks: task.subtasks.map(sub => sub.id === subtaskId ? { ...sub, completed: !sub.completed } : sub) }); };
  const handleDeleteSubtask = (subtaskId) => { handleUpdate({ ...task, subtasks: task.subtasks.filter(sub => sub.id !== subtaskId) }); };
